...
串行数据输入由SYNC开始，之后的24个SCLK载入数据。设计SCLK采用20MHz时钟，以下参数采用V~LOGIC~为3.3V情况：
...
| ---- | ------------------------------ | ------- | ---------------- |
| t1   | SCLK周期                       | > 20ns  | 50ns             |
| t4   | SYNC下降沿到下一个SCLK下降沿   | > 10ns  | 25ns（5sys_clk） |
| t7   | 最后一个SCLK下降沿到SYNC上升沿 | > 10ns  | 25ns             |
| t8   | SYNC高电平时间                 | > 20ns  | 500ns（10SCLK）  |
| t9   | 两次SYNC上升沿之间间隔         | > 830ns | > 850ns          |
| t10  | SYNC下降沿到前一个SCLK下降沿   | > 10ns  | 25ns             |
| t5   | 数据到SCLK下降沿建立时间       | > 5ns   | 25ns             |
| t6   | 数据到SCLK下降沿保持时间       | > 5ns   | 25ns             |
| t15  | RESET低电平时间                | > 30ns  | 200ns(4SCLK)     |
